...
response.matrix.rel.inf <- lepmac.response.matrix %>% 
  ...
  mutate(Predictor_Name = fct_relevel(Predictor_Name, 
                                      c( "Lake Depth", "Degree Days", 
                                         "Lake Area", "Surface Temperature",
                                         "Bluegill CPUE",
...
ggsave(filename = "Figures/partial_dependency_reduced.tiff",
       partial.plot,
       dpi = 300,
       width = 200,
       height = 150,
...
ggsave(filename = "Figures/rel_inf_reduced.tiff",
       rel.inf.plot,
       dpi = 301,
       width = 200,
       height = 150,
...
write.csv(brt.sum.stats, file = "Tables/brt_sum_stats_reduced.csv", row.names = F)
